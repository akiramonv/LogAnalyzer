...
| **Причина** | Цепочка `Caused by` разворачивается до первопричины, учитываются каскад ошибок и предшествующие таймауты; результат — гипотеза с уверенностью, доказательствами (`e5`, `e4`) и рекомендацией. |
...
- слева — поле для вставки отрывка (или перетаскивания файла) и вкладка «Файл или каталог»
  для больших логов, которые читаются прямо с диска;
- фильтры: только с ошибками, traceId, минимальный уровень, топ N инцидентов;
- справа — таймлайн инцидента с причиной, поиском и раскрывающимися стеками;
- кнопки «Скачать JSON» и «Скачать HTML» для отчёта.
...
Готово и покрыто тестами (91 тест): разбор всех перечисленных форматов, корреляция,
таймлайн, правила, анализ причин, четыре формата отчёта, CLI и локальный веб-интерфейс.
